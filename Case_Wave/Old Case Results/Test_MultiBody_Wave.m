...
% Pass true to run the configured free-surface domain through HydroMain.
    if nargin<1, run_full_domain=false; end
    ...
    if run_full_domain
        ...
        report=struct('mode','full_domain','passed',true,'results',results); return;
    end
    cfg=read_config(config_file); coarse_file=fullfile(here,'two_sphere_smoke_body.bmf');
    generate_body_bmf(coarse_file,10,0,0,2); bodies=cell(2,1);
    for b=1:2
        local=read_bmf(coarse_file); bodies{b}=transform_body_mesh(local,cfg.bodies(b));
        bodies{b}.cg=cfg.mass_props(b).cg; bodies{b}.body_id=b;
    end
    domain=struct('body_list',{bodies},'fs',[],'seabed',[],'farfield',[]); geom=merge_domain_geometry(domain);
    ...
    fprintf('>>> Multi-body regression passed: %d panels, BEM %dx%d, 12 radiation modes.\n', ...
