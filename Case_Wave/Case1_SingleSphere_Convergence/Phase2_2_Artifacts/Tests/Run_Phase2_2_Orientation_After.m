...
% visible and failing until separately authorized.
    ...
    rows = repmat(empty_after_result(), 5 + numel(unaffectedIds), 1);
    ...
    cfg = minimal_assembly_config();
    [~, ~, assemblyInfo] = assemble_rankine_matrix(6, centers, normals, ...
        panels, stats, 1.0, cfg.grav, cfg, [1, 1]);
    assemblyContractPass = ...
        strcmp(assemblyInfo.double_layer_source_normal_convention, ...
        orientation.convention) && ...
        isequal(assemblyInfo.double_layer_component_signs, ...
        orientation.componentSigns) && ...
        strcmp(assemblyInfo.double_layer_orientation_hash, ...
        orientation.signatureHash);
    rows(4) = make_after_result('T2_ASSEMBLY_INFO_CONTRACT', ...
        double(~assemblyContractPass), 0.0, true, 'REPAIRED_PASS', ...
        'A real production assembly exposes the effective convention, signs, and hash.');

    meshAudit = minimal_mesh_audit();
    [~, cacheSpecification] = build_potential_cache_key(cfg, meshAudit, ...
        1.0, ones(6, 2), [1, 1], codeVersion, ...
        assemblyInfo.outer_absorbing_boundary, ...
        assemblyInfo.boundary_operator_state);
    cacheOptions = cacheSpecification.solverOptions;
    cacheContractPass = ...
        strcmp(cacheOptions.doubleLayerSourceNormalConvention, ...
        orientation.convention) && ...
        isequal(cacheOptions.doubleLayerComponentSigns, ...
        orientation.componentSigns) && ...
        strcmp(cacheOptions.doubleLayerOrientationHash, ...
        orientation.signatureHash);
    rows(5) = make_after_result('T2_CACHE_MANIFEST_CONTRACT', ...
        double(~cacheContractPass), 0.0, true, 'REPAIRED_PASS', ...
        'The production cache manifest binds the named convention, signs, and hash.');
    for testIndex = 1:numel(unaffectedIds)
        ...
        rows(5 + testIndex) = make_after_result(unaffectedIds(testIndex), ...
...
function cfg = minimal_assembly_config()
    cfg = struct('isx', 0, 'isy', 0, 'water_depth', 10.0, ...
        'grav', 9.80665, 'rho', 1025.0, 'z_tol', 1e-3, ...
        'n_bodies', 1, 'calc_modes', 1:6, ...
        'run', struct('irad', 1, 'idiff', 1), ...
        'wave', struct('headings', 0.0), ...
        'fs', struct('nr_near', 1, 'nr_sponge', 1, ...
        'r_inner', 1.0, 'r_outer', 2.0, 'sponge_ratio', 1.1, ...
        'nz_farfield', 1, 'mu0', 0.35));
end

function meshAudit = minimal_mesh_audit()
    component = struct('hash', repmat('0', 1, 64));
    meshAudit = struct('body', component, 'freeSurface', component, ...
        'bottom', component, 'outerBoundary', component, ...
        'geometry', component);
end
