...
%% Stage 1: Initialize inputs and dependencies
    ...
    if isstruct(run_solver) && isfield(run_solver, 'radiationRHSAudit')
        [rhs, rhsInfo] = benchmark_radiation_rhs(run_solver.singleLayerMatrix, ...
            run_solver.omega, run_solver.generalizedNormals, run_solver.modeIndices);
        benchmark = rhsInfo;
        benchmark.rhs = rhs;
        return;
    end
    validateattributes(run_solver, {'logical','numeric'}, {'scalar'});
...
%% Stage 2: Run the core calculation
    ...
    B33Pressure = cellfun(@(r)r.pressure_damping(3,3),cases);
    B33HaskindCandidate = cellfun(@(r) ...
        r.haskind_damping_candidate(3,3),cases);
    F3 = cellfun(@(r)r.excitation(3),cases);
    speedup = runtime(1) ./ max(runtime,eps);
    relA = abs(A33 - A33(1)) / max(abs(A33(1)),eps);
    relBPressure = abs(B33Pressure - B33Pressure(1)) / ...
        max(abs(B33Pressure(1)),eps);
    relBHaskindCandidate = abs(B33HaskindCandidate - ...
        B33HaskindCandidate(1)) / max(abs(B33HaskindCandidate(1)),eps);
    relF = abs(F3 - F3(1)) / max(abs(F3(1)),eps);
    summary = table(labels,unknowns,equivalent,runtime,speedup,A33, ...
        B33Pressure,B33HaskindCandidate,F3,relA,relBPressure, ...
        relBHaskindCandidate,relF, ...
'VariableNames',{'domain','unknowns','full_equivalent_panels','runtime_s','speedup', ...
'A33','B33_pressure','B33_haskind_candidate','F3', ...
'A33_rel_difference','B33_pressure_rel_difference', ...
'B33_haskind_candidate_rel_difference','F3_rel_difference'});
...
%% Stage 2: Run the core calculation
    ...
    for groupIndex = 1:ng
        ...
        [radiationRHS, radiationRHSInfo{groupIndex}] = ...
            benchmark_radiation_rhs(S, omega, nj(:,modes), modes);
        [phi,factors] = solve_complex_system(K,radiationRHS,[]);
...
'mode_parity',mode_parity,'full_equivalent_dofs',geom.total_panels * 2^(cfg.isx + cfg.isy));
    ...
    [haskindDampingCandidate,haskindCandidateDiagnostics] = ...
        compute_haskind_damping_candidate(phi_rad,nj,centers,normals, ...
        areas,omega,cfg,mode_parity,72);
    diagnostics.pressure_damping = pressure_damping;
    diagnostics.haskind_candidate = struct( ...
        'damping', haskindDampingCandidate, ...
        'validationStatus', 'NOT_VALIDATED', ...
        'diagnostics', haskindCandidateDiagnostics);
    result = struct('stats',domain.stats,'symmetry',symmetry, ...
        'added_mass',A,'pressure_damping',pressure_damping, ...
        'haskind_damping_candidate',haskindDampingCandidate, ...
        'excitation',force,'diagnostics',diagnostics, ...
        'assembly_info',{assembly});
    result.radiation_rhs_info = radiationRHSInfo;
end

function [rhs, info] = benchmark_radiation_rhs( ...
        singleLayerMatrix, omega, generalizedNormals, modeIndices)
% BENCHMARK_RADIATION_RHS Route the active benchmark through production RHS.

    [rhs, sourceFlux, info] = build_rankine_radiation_rhs( ...
        singleLayerMatrix, omega, generalizedNormals, modeIndices);
    newOracle = -singleLayerMatrix * sourceFlux;
    oldOracle = singleLayerMatrix * sourceFlux;
    info.benchmarkNewSignRelativeError = relative_matrix_error(rhs, newOracle);
    info.benchmarkOldSignRelativeError = relative_matrix_error(rhs, oldOracle);
    info.benchmarkNewSignPass = info.benchmarkNewSignRelativeError <= 1e-13;
    info.benchmarkOldSignPass = info.benchmarkOldSignRelativeError <= 1e-10;
    assert(info.benchmarkNewSignPass && ~info.benchmarkOldSignPass, ...
        'CRESTU:BenchmarkRadiationRHSSign', ...
        'Active symmetry benchmark failed the radiation RHS sign oracle.');
end

function value = relative_matrix_error(first, second)
