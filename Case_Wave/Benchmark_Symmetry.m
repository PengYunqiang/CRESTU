...
% independently generated panel layouts cannot contaminate the comparison.
    ...
    if ~run_solver
        ...
        loaded=load(output_file,'benchmark'); benchmark=loaded.benchmark; return;
    end
    ...
    labels={'Full';'Half-X';'Quarter-XY'}; cases=cell(3,1); runtime=zeros(3,1);
    for k=1:3
        timer=tic; cases{k}=solve_domain_once(domains{k}); runtime(k)=toc(timer);
    end
    unknowns=cellfun(@(r)r.stats.total_dofs,cases); equivalent=cellfun(@(r)r.symmetry.full_equivalent_dofs,cases);
    A33=cellfun(@(r)r.added_mass(3,3),cases); B33=cellfun(@(r)r.damping(3,3),cases);
    F3=cellfun(@(r)r.excitation(3),cases); speedup=runtime(1)./max(runtime,eps);
    relA=abs(A33-A33(1))/max(abs(A33(1)),eps); relB=abs(B33-B33(1))/max(abs(B33(1)),eps);
    ...
    save(output_file,'benchmark'); disp(summary);
end

function expanded=expand_domain(base,reflect_x,reflect_y,target_isx,target_isy)
    expanded=base;
    for b=1:numel(base.body_list)
        expanded.body_list{b}=expand_mesh_by_symmetry(base.body_list{b},reflect_x,reflect_y);
    end
    names={'fs','seabed','farfield'};
    for k=1:numel(names)
        name=names{k};
        if ~isempty(base.(name)), expanded.(name)=expand_mesh_by_symmetry(base.(name),reflect_x,reflect_y); end
    end
    expanded.cfg.isx=target_isx; expanded.cfg.isy=target_isy;
    ...
    expanded.stats=domain_stats(expanded); expanded.geometry=merge_domain_geometry(expanded);
end
...
function result=solve_domain_once(domain)
    cfg=domain.cfg; geom=domain.geometry; omega=cfg.freq.omegas(1); heading=cfg.wave.headings(1);
    nb=domain.stats.total_body_panels; ndof=6*cfg.n_bodies;
    centers=geom.centers(1:nb,:); normals=geom.normals(1:nb,:); areas=geom.areas(1:nb);
    ...
    phi_rad=complex(zeros(nb,ndof)); force=complex(zeros(ndof,1)); assembly=cell(ng,1);
    for q=1:ng
        parity=group_parity(q,:); modes=find(all(mode_parity==parity,2));
        [K,S,assembly{q}]=assemble_rankine_matrix(geom.total_panels,geom.centers,geom.normals, ...
