...
%% Stage 1: Validate Inputs and Initialize the Algorithm
    ...
    if ~isfield(criteria,'body_max_ar')
        criteria.body_max_ar = 4.0;
    end % Maximum body-panel aspect ratio.
    if ~isfield(criteria,'fs_near_max_ar')
        criteria.fs_near_max_ar = 4.5;
    end % Near-field FS aspect ratio.
    if ~isfield(criteria,'fs_sponge_max_ar')
        criteria.fs_sponge_max_ar = 12.0;
    end % Sponge aspect ratio.
    if ~isfield(criteria,'max_skewness')
        criteria.max_skewness = 55.0;
    end % Maximum orthogonality error, deg.
    if ~isfield(criteria,'min_angle')
        criteria.min_angle = 35.0;
    end % Minimum corner angle, deg.
    if ~isfield(criteria,'max_angle')
        criteria.max_angle = 145.0;
    end % Maximum corner angle, deg.
    if ~isfield(criteria,'max_warp_deg')
        criteria.max_warp_deg = 8.0;
    end % Maximum panel warping, deg.
...
% Type 1: Body, Type 2: FS, Type 4: Seabed, Type 5: Farfield
    for bodyIndex = 1:domain.cfg.n_bodies
        part_count = part_count + 1;
        parts{part_count} = domain.body_list{bodyIndex};
        names{part_count} = sprintf('Body_%d', bodyIndex);
...
'Boundary component','Panel count','Maximum aspect ratio','Maximum skew angle(deg)','Minimum angle(deg)','Maximum warpage(deg)','Poor-panel rate');
    ...
    for componentIndex = 1:length(parts)
        m = parts{componentIndex};
        np = m.n_panels;
        verts = m.vertices; % [np x 4 x 3]
        centers = m.centers;
        ptype = part_types(componentIndex);
        ...
        for i = 1:np
            panelVertices = squeeze(verts(i, :, :)); % [4 x 3]

            edgeVectors = [panelVertices(2, :) - panelVertices(1, :); ...
                panelVertices(3, :) - panelVertices(2, :); ...
                panelVertices(4, :) - panelVertices(3, :); ...
                panelVertices(1, :) - panelVertices(4, :)];
            lens = sqrt(sum(edgeVectors .^ 2, 2));
            ...
            for k = 1:4
                ...
                e_in = -edgeVectors(k_prev, :) / max(lens(k_prev), 1e-12);
                e_out = edgeVectors(k, :) / max(lens(k), 1e-12);
            ...
            n1 = cross(edgeVectors(1, :), edgeVectors(2, :));
            n2 = cross(edgeVectors(3, :), edgeVectors(4, :));
        ...
        fprintf('%-13s | %-6d | %-8.2f | %-8.2f | %-8.2f | %-8.2f | %-7.2f%%\n', ...
                names{componentIndex}, np, max(aspect_ratios), max(skew_angles), min(min_angles), max(warps), bad_rate);

        report.(names{componentIndex}).aspect_ratio = aspect_ratios;
        report.(names{componentIndex}).skewness = skew_angles;
        report.(names{componentIndex}).warping = warps;
        report.(names{componentIndex}).is_bad = is_bad;
        report.(names{componentIndex}).bad_rate = bad_rate;
...
%% Stage 1: Validate Inputs and Initialize the Algorithm
    ...
    fprintf('[WARN] BEM preprocessing recommendations:\n');
    fnames = fieldnames(report);
    any_issue = false;
    for i = 1:length(fnames)
        componentReport = report.(fnames{i});
        if componentReport.bad_rate > 5.0
            any_issue = true;
            fprintf(' * [%s] poor-panel fraction %.1f%%.', fnames{i}, componentReport.bad_rate);
            if max(componentReport.aspect_ratio) > criteria.fs_sponge_max_ar
                fprintf(' Radial stretching is excessive; reduce sponge_ratio or add radial sponge-layer segments.\n');
            elseif max(componentReport.skewness) > criteria.max_skewness
