...
% GENERATE_MULTIBODY_FREE_SURFACE_BMF Execute the documented generate_multibody_free_surface_bmf operation.
%
% Syntax:
...
%   waterlines      : [cell] Ordered waterline structures for all bodies.
...
% ==========================================
%GENERATE_MULTIBODY_FREE_SURFACE_BMF Constrained triangular FS with N holes.
% Triangles are stored as valid degenerate quads (vertex 4 repeats vertex 3).
    n_body=numel(waterlines); outer_n=max(48,sum(cellfun(@(w)w.n_pts,waterlines)));
    theta=linspace(0,2*pi,outer_n+1).'; theta(end)=[];
    outer=cfg.fs.r_outer*[cos(theta),sin(theta)]; points=outer; constraints=[(1:outer_n).',[2:outer_n,1].'];
    offset=outer_n;
    for b=1:n_body
        xy=waterlines{b}.nodes; nb=size(xy,1); points=[points;xy]; %#ok<AGROW>
        ids=offset+(1:nb); constraints=[constraints;ids.',ids([2:end,1]).']; %#ok<AGROW>
        offset=offset+nb;
    end
    ...
    [gx,gy]=meshgrid(axis_values,axis_values); candidate=[gx(:),gy(:)];
    keep=sum(candidate.^2,2)<(0.985*cfg.fs.r_outer)^2;
    for b=1:n_body
        keep=keep&~inpolygon(candidate(:,1),candidate(:,2), ...
            waterlines{b}.nodes(:,1),waterlines{b}.nodes(:,2));
    end
    ...
    dt=delaunayTriangulation(points,constraints); tri=dt.ConnectivityList; xy=dt.Points;
    ...
    tri=tri(keep,:); np=size(tri,1); vertices=zeros(np,4,3);
    for q=1:3, vertices(:,q,1:2)=xy(tri(:,q),:); end
    vertices(:,4,:)=vertices(:,3,:); centers=zeros(np,3); centers(:,1:2)=centroids(keep,:);
    normals=repmat([0,0,1],np,1); areas=zeros(np,1); e1=zeros(np,3); e2=zeros(np,3);
    for p=1:np
        p1=reshape(vertices(p,1,:),1,3); p2=reshape(vertices(p,2,:),1,3); p3=reshape(vertices(p,3,:),1,3);
        areas(p)=0.5*norm(cross(p2-p1,p3-p1)); tangent=(p2-p1)/norm(p2-p1);
        e1(p,:)=tangent; e2(p,:)=cross([0,0,1],tangent);
    end
    radii=sqrt(sum(centers(:,1:2).^2,2)); mu=zeros(np,1); mask=radii>cfg.fs.r_inner;
