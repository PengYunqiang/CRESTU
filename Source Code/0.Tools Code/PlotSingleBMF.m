...
%% Stage 2: Read the BMF or GDF Mesh
...
%% Stage 3: Evaluate Panel Centers and Unit Normals
...
normalVectors = cross(V12, V13, 1);

% Normalize each vector to unit magnitude.
N_mag = sqrt(sum(normalVectors .^ 2, 1));
N_mag(N_mag < 1e-12) = 1;
N_unit = normalVectors ./ N_mag;
Nx = N_unit(1, :);
Ny = N_unit(2, :);
Nz = N_unit(3, :);

%% Stage 4: Render the Mesh
