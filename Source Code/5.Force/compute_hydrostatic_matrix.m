...
'displaced_volume', 0,'center_of_buoyancy', zeros(1, 3),'block', zeros(6));
    details = repmat(template, bodyCount, 1);
    for bodyIndex = 1:bodyCount
        mesh = body_list{bodyIndex};
        cg = reshape(cfg.mass_props(bodyIndex).cg, 1, 3);
        wl = extract_waterline(mesh, cfg.z_tol);
        xy = wl.nodes;
        if cfg.isx || cfg.isy
            wl = complete_waterline_by_symmetry(wl, cfg.isx, cfg.isy);
            xy = wl.nodes;
        end
        ...
        if ~(isfinite(volume) && volume > 0)
            error('CRESTU:HydrostaticVolume','Body %d has no positive displaced volume.', bodyIndex);
        end
        zb = hs.center_of_buoyancy(3);
        mass = cfg.mass_props(bodyIndex).mass;
        block = zeros(6);
        rg = cfg.rho * cfg.grav;
        block(3, 3) = rg * Awp;
        block(3, 4) = rg * Qy;
        ...
        block(4, 5) = -rg * Ixy;
        block(5, 4) = block(4, 5);
        idx = (bodyIndex - 1) * 6 + (1:6);
        C(idx, idx) = block;
        details(bodyIndex) = struct('body_id', cfg.bodies(bodyIndex).id,'waterplane_area', Awp, ...
...
%% Stage 1: Validate Inputs and Initialize the Algorithm
    ...
    if size(xy, 1) < 3
        error('CRESTU:WaterplanePolygon','Waterline needs at least three vertices.');
    end
    xy = xy - reference;
    x = xy(:, 1);
    y = xy(:, 2);
    xn = x([2:end, 1]);
    yn = y([2:end, 1]);
    cross_terms = x .* yn - xn .* y;
    signed_A = 0.5 * sum(cross_terms);
    if abs(signed_A) < eps
        error('CRESTU:WaterplaneArea','Waterplane polygon area is zero.');
    end
    orientationSign = sign(signed_A);
    A = abs(signed_A);
    Qx = orientationSign * sum((x + xn) .* cross_terms) / 6;
    Qy = orientationSign * sum((y + yn) .* cross_terms) / 6;
    Ixx = orientationSign * sum((y .^ 2 + y .* yn + yn .^ 2) .* cross_terms) / 12;
    Iyy = orientationSign * sum((x .^ 2 + x .* xn + xn .^ 2) .* cross_terms) / 12;
    Ixy = orientationSign * sum((2 * x .* y + x .* yn + xn .* y + 2 * xn .* yn) .* cross_terms) / 24;
