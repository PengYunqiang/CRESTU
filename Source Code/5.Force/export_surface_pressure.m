...
'centers', mesh.centers,'vertices', mesh.vertices);
    ...
    fprintf('[OK] Surface pressure exported: %s\n', filename);
end
...
%% Stage 1: Validate Inputs and Initialize the Algorithm
    ...
    if fid < 0
        error('CRESTU:PressureOpen','Cannot create %s.', filename);
    end
    cleanup = onCleanup(@()fclose(fid));
    np = mesh.n_panels;
    vertices = reshape(mesh.vertices, np, 4, 3);
    fprintf(fid,'# vtk DataFile Version 3.0\nCRESTU complex dynamic pressure\nASCII\n');
    fprintf(fid,'DATASET UNSTRUCTURED_GRID\nPOINTS %d double\n', 4 * np);
    for panelIndex = 1:np
        for vertexIndex = 1:4
            fprintf(fid,'%.15g %.15g %.15g\n', vertices(panelIndex, vertexIndex, 1), vertices(panelIndex, vertexIndex, 2), vertices(panelIndex, vertexIndex, 3));
        end
    end
    fprintf(fid,'CELLS %d %d\n', np, 5 * np);
    for panelIndex = 1:np
        vertexOffset = 4 * (panelIndex - 1);
        fprintf(fid,'4 %d %d %d %d\n', vertexOffset, vertexOffset + 1, vertexOffset + 2, vertexOffset + 3);
    end
